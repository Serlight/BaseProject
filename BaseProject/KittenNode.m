...
- (instancetype)initWithKittenOfSize:(CGSize)size {
    ...
    if (self) {
        ...
        _imageNode.URL = [NSURL URLWithString:[NSString stringWithFormat:@"http://placekitten.com/%zd/%zd", (NSInteger)roundl(_kittenSize.width), (NSInteger)roundl(_kittenSize.height)]];
        [_imageNode addTarget:self action:@selector(toggleNodesSwap) forControlEvents:ASControlNodeEventTouchUpInside];
        
        [self addSubnode:_imageNode];
        
        _textNode = [[ASTextNode alloc] init];
        _textNode.attributedString = [[NSAttributedString alloc] initWithString:[self kittyIpsum] attributes:[self textStyle]];
...
- (ASLayoutSpec *)layoutSpecThatFits:(ASSizeRange)constrainedSize {
    ...
    [stackSpec setChildren:!_swppedTextAndImage ? @[_imageNode, _textNode] : @[_textNode, _imageNode]];
...
- (void)toggleNodesSwap {
    _swppedTextAndImage = !_swppedTextAndImage;
